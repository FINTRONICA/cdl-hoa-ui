...
// Helper function for alphanumeric (50,0) - max 50 characters, alphanumeric with hyphens and underscores
...
// Helper function for all characters (50,0) - max 50 characters, any character allowed (including special chars)
const allCharactersString = (maxLength: number = 50, optional: boolean = false) => {
  if (optional) {
    return z.string().max(maxLength, `Maximum ${maxLength} characters allowed`).or(z.literal(''))
  }
  return z
    .string()
    .min(1, 'Required field')
...
export const budgetMasterStep1Schema = z.object({
  ...
  groupNameId: allCharactersString(50, true),
