...
export const CapitalPartnerStep1Schema = z.object({
  ...
  investorType: z.string()
    .min(1, 'Capital Partner Type is required'),
  
  investorFirstName: z.string()
    .min(1, 'Capital Partner Name is required')
    .max(50, 'Capital Partner Name must be 50 characters or less')
    .regex(/^[A-Za-z\s]+$/, 'Capital Partner Name can only contain alphabets'),
  ...
  investorId: z.string()
    .min(1, 'Capital Partner Reference ID is required'),
  
  investorIdType: z.string()
    .min(1, 'Capital Partner ID Type is required'),
  
  idNumber: z.string()
    .min(1, 'ID Number is required')
    .max(15, 'ID Number must be 15 characters or less')
    .regex(/^[A-Za-z0-9]+$/, 'ID Number can only contain alphanumeric characters'),
  ...
  accountContact: z.string()
    .max(15, 'Account Contact Number must be 15 characters or less')
    .regex(/^[\d]*$/, 'Account Contact Number can only contain numerical characters')
    .optional()
    .or(z.literal('')),
...
export const CapitalPartnerStep2Schema = z
  .object({
    ...
    developerIdInput: z.string().min(1, 'Developer ID is required'),
    developerNameInput: z.string().min(1, 'Developer Name is required'),
    unitNoQaqood: z
      .string()
    ...
    paidInEscrow: z
      ...
      .regex(/^\d*(\.\d+)?$/, 'Amount Paid Within Escrow must be a valid number')
      .optional()
      .or(z.literal('')),
    paidOutEscrow: z
      .string()
      .max(15, 'Amount Paid Out of Escrow must be 15 characters or less')
      .regex(/^\d*(\.\d+)?$/, 'Amount Paid Out of Escrow must be a valid number')
      .optional()
      .or(z.literal('')),
    totalPaid: z
      .string()
      ...
      if (!Number.isNaN(within) && !Number.isNaN(out) && !Number.isNaN(total)) {
        ...
        if (sum !== totalRounded) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ['totalPaid'],
            message: 'Total Amount Paid must equal Within Escrow + Out of Escrow',
          })
        }
      }
    }
...
export const CapitalPartnerStep5Schema = z.object({
  ...
  accountNumber: z.string()
    ...
    .regex(/^[A-Za-z0-9]+$/, 'Account number can only contain alphanumeric characters'),
  ...
  accountHolderName: z.string()
    ...
    .regex(/^[A-Za-z\s\u0600-\u06FF]+$/, 'Account holder name can only contain letters and spaces'),
  ...
  branchCode: z.string()
    ...
    .regex(/^[A-Za-z0-9]*$/, 'Branch code can only contain alphanumeric characters')
    .optional()
    .or(z.literal('')),
