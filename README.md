...
- **Asset Registry Management**: Developer/Asset Registry onboarding with multi-step forms
- **owner-registryr Management**: Investor and owner-registryr management
...
│   │   │   ├── asset-registry/    # Asset Registry/developer management
│   │   │   │   ├── page.tsx      # List page (/asset-registry)
│   │   │   │   ├── new/          # Create new (/asset-registry/new)
│   │   │   │   └── [id]/         # Detail pages
│   │   │   │       └── step/     # Multi-step forms
│   │   │   │           └── [stepNumber]/page.tsx
│   │   │   ├── management-firms/  # Asset Registry assets management
│   │   │   │   ├── page.tsx      # List page (/management-firms)
│   │   │   │   ├── new/          # Create new (/management-firms/new)
│   │   │   │   └── [id]/         # Detail pages (/management-firms/{id})
│   │   │   └── owner-registry/      # owner-registryr management
│   │   │       ├── page.tsx      # List page (/owner-registry)
│   │   │       ├── new/          # Create new (/owner-registry/new)
│   │   │       └── [id]/         # Detail pages
│   │   │           ├── page.tsx   # owner-registryr details
│   │   │           └── step/      # Multi-step forms
│   │   │               └── [stepNumber]/page.tsx
...
- `(entities)`: Groups entity management routes without adding `/entities` to the URL
  - `/asset-registry` instead of `/entities/asset-registry`
  - `/management-firms` instead of `/entities/management-firms`
  - `/owner-registry` instead of `/entities/owner-registry`

#### **Entity Management Routes**

**Asset Registrys** (also referred to as "Developers" in the UI)

- **List**: `/asset-registry` - Main Asset Registrys list page
- **New**: `/asset-registry/new` - Create new Asset Registry form
- **Details**: `/asset-registry/{id}/step/{stepNumber}` - Multi-step forms
  - **Edit Mode**: `/asset-registry/{id}/step/{stepNumber}?editing=true`
  - **View Mode**: `/asset-registry/{id}/step/{stepNumber}?mode=view`

**Asset Registry Assets** (also referred to as "Projects" in the UI)

- **List**: `/management-firms` - Assets list page
- **New**: `/management-firms/new` - Create new asset
- **Details**: `/management-firms/{id}` - Asset details

**owner-registryrs** (also referred to as "Investor" in the UI)

- **List**: `/owner-registry` - owner-registryrs list
- **New**: `/owner-registry/new` - Create new owner-registryr form
- **Details**: `/owner-registry/{id}` - owner-registryr details
  - **Step Forms**: `/owner-registry/{id}/step/{stepNumber}` - Multi-step forms
...
```
Dashboard → Asset Registrys List → Asset Registry Details → Edit/View
    ↓              ↓                      ↓
/asset-registry → /asset-registry/{id}/step/1 → /asset-registry/{id}/step/2
```
...
- Multiple label stores for different entities:
  - Sidebar labels
  - Asset Registry labels
  - owner-registryr labels
  - Workflow labels
  - etc.
...
- **Asset Registrys**: `buildPartnerService.ts`
- **owner-registryrs**: `capitalPartnerService.ts`
...
- **`DeveloperStepper`**: Multi-step developer onboarding (for Asset Registrys)
...
- **`InvestorStepper`**: Multi-step investor/owner-registryr onboarding
...
- Domain-specific schemas (developer, owner-registryr, project, transaction, workflow, etc.)
...
- **Asset Registry Labels**: Asset Registry form labels
- **owner-registryr Labels**: owner-registryr form labels
...
- `useBuildPartners.ts`: Asset Registry data fetching
